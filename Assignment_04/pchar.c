...
static __init int pchar_init(void) {
    ...
    for(i=0;i<devcnt;i++){
       sema_init(&devices[i].s,1);
    }
    printk(KERN_INFO "%s: mutex_init() initilized mutex lock for all devices\n", THIS_MODULE->name);
    return 0;
cdev_add_failed:
    for(i=i-1; i>=0; i--)
...
static int pchar_open(struct inode *pinode, struct file *pfile) {
    ...
    printk(KERN_INFO "%s:up() operation started pchar%d in process %d (%s).\n", THIS_MODULE->name,MINOR(pdev->devno),get_current()->pid,get_current()->comm);
    for(int i =0;i<devcnt;i++)
        down(&devices[i].s);
    printk(KERN_INFO "%s:pchar%d up() operation is done by process %d (%s).\n", THIS_MODULE->name,MINOR(pdev->devno),get_current()->pid,get_current()->comm);
...
static int pchar_close(struct inode *pinode, struct file *pfile) {
    ...
    (void)pdev;
    printk(KERN_INFO "%s:down() operation started pchar%d in process %d (%s).\n", THIS_MODULE->name,MINOR(pdev->devno),get_current()->pid,get_current()->comm);
    for(int i =0;i<devcnt;i++)
        up(&devices[i].s);
    printk(KERN_INFO "%s:pchar%d down() operation is done by process %d (%s).\n", THIS_MODULE->name,MINOR(pdev->devno),get_current()->pid,get_current()->comm);
    return 0;
}
